	...
	for scanner.Scan() {
		...
		if inMultiLineBlock {
			if trimmedLine == blockEndChar {
				// 遇到块结束符，将拼接好的内容存入配置。
				// 对于数组和 Map，我们都将多行内容拼接成一个长字符串，
				// 后续的 parseSlice 和 parseMap 函数会负责按逗号分割。
				newConfig[multiLineKey] = multiLineContent
				// 重置状态。
